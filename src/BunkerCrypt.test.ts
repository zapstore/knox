...
Deno.test('encrypt and decrypt', () => {
  for (let i = 0; i < vectors.length; i++) {
    const [password, secret, logn, ksb, ncryptsec] = vectors[i];

    const sec = hexToBytes(secret);
    const crypt = new BunkerCrypt(password);

    const there = crypt.encrypt(sec, logn, ksb);
    const back = crypt.decrypt(there);
    const again = crypt.decryptKey(ncryptsec);

    assertEquals(back, again);
    assertEquals(again, sec);
  }
});

const vectors: [string, string, number, 0x00 | 0x01 | 0x02, `ncryptsec1${string}`][] = [
  [
    '.ksjabdk.aselqwe',
